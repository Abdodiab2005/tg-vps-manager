  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (parts.length > 1) {
      if (options.parse_mode === "Markdown") {
        ...
        messageText = `📄 <b>الجزء ${i + 1}/${parts.length}</b>:\n${part}`;
      }
    }
    ...
    try {
      ...
      await bot.sendMessage(
        chatId,
        `❌ خطأ في إرسال جزء من الرسالة: ${i + 1}/${parts.length}`
      );
    }
  }
}
...
🔧 <b>مرحباً بك في بوت إدارة VPS</b>
...
bot.onText(/\/run (.+)/, (msg, match) => {
  ...
  bot.sendMessage(chatId, `🔄 تنفيذ الأمر: <pre>${command}</pre>`, {
    parse_mode: "HTML",
  });
